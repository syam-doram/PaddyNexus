...
export default function FarmerHarvest() {
  ...
  return (
    <div className="relative flex h-full w-full flex-col bg-[#F8FAFC] dark:bg-[#0F172A] font-display text-slate-900 dark:text-slate-100">
      ...
      <main className={`flex-1 max-w-6xl mx-auto w-full p-4 md:p-6 pb-32 transition-all ${isLocked ? 'blur-sm grayscale-[0.5] pointer-events-none opacity-60' : ''}`}>
            ...
            <div className="lg:col-span-4 space-y-6">
                ...
                <div className="p-8 bg-slate-900 dark:bg-slate-800 rounded-[40px] text-white">
                    ...
                    <p className="text-xs font-medium text-slate-400 leading-relaxed">
                        Accurate time readings are critical for seasonal audit integrity. Ensure GPS sync on mobile devices if available.
                    </p>
                </div>
            </div>
            ...
            <div className="lg:col-span-3 space-y-6">
                <section className="bg-white dark:bg-surface-dark p-6 md:p-8 rounded-[32px] md:rounded-[40px] border border-slate-100 dark:border-white/5 shadow-sm h-full flex flex-col">
                    ...
                    <button 
                        onClick={handleSave}
                        disabled={submitting}
                        className="w-full h-20 mt-12 bg-primary hover:bg-[#22c55e] text-background-dark rounded-[32px] font-black text-lg uppercase tracking-tight shadow-2xl shadow-primary/20 active:scale-95 transition-all flex items-center justify-center gap-3"
                    >
                        {submitting ? 'Executing...' : <><Save className="w-5 h-5" /> Commit Entry</>}
                    </button>
                </section>
            </div>
        </div>
