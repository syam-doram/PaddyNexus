...
export default function MillSettlementList() {
  ...
  const stats = useMemo(() => {
    ...
    const totalSettled = mills.filter(m => m.is_settled === 1).reduce((sum, m) => sum + m.totalPaid, 0);
    ...
    return { totalSettled, totalOutstanding, totalThroughput };
  }, [mills]);
    ...
    <div className="h-screen w-full bg-[#f8fafc] dark:bg-[#020617] font-display flex flex-col overflow-hidden text-slate-900 dark:text-slate-100">
      ...
      <main className="flex-1 overflow-y-auto no-scrollbar p-4 md:p-10 pb-32 md:pb-10 space-y-8 md:space-y-12">
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
          ...
          <div className="bg-emerald-50/50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-500/20 p-6 md:p-10 rounded-3xl md:rounded-[48px] flex flex-col justify-center relative shadow-sm overflow-hidden group hover:shadow-2xl hover:shadow-emerald-500/10 transition-all duration-500">
            <div className="absolute top-0 right-0 p-6 md:p-10 opacity-10 group-hover:scale-110 transition-transform duration-500 text-emerald-500">
               <ShieldCheck className="w-24 h-24 md:w-32 md:h-32" />
            </div>
            <div className="relative z-10">
                <div className="flex items-center gap-3 md:gap-4 mb-4 md:mb-6 text-emerald-600 dark:text-emerald-400">
                  ...
                  <div>
                    <h3 className="text-[10px] md:text-xs font-black uppercase tracking-[0.2em] leading-none mb-1">Settled Capital</h3>
                    <p className="text-[8px] md:text-[9px] font-bold text-emerald-500/60 uppercase tracking-widest">Aggregate Verification</p>
                  </div>
                </div>
                <p className="text-3xl md:text-5xl font-[1000] text-emerald-600 dark:text-emerald-400 tracking-tighter italic leading-none mb-3 md:mb-4">
                  {formatCurrency(stats.totalSettled)}
                </p>
                <div className="flex items-center gap-3">
                  <span className="px-2.5 py-1 bg-emerald-500 text-white text-[8px] md:text-[9px] font-black uppercase tracking-widest rounded-full">{mills.filter(m => m.status === 'SETTLED').length} Mills</span>
                  <p className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-widest">Fully Settled</p>
                </div>
            </div>
          </div>
            ...
            {filteredMills.map((mill, i) => (
              ...
              >
                ...
                <div className="flex items-start justify-between mb-4 md:mb-6">
                  ...
                  <div className={`px-2.5 py-1 md:px-3 md:py-1.5 rounded-lg md:rounded-xl text-[8px] md:text-[9px] font-black uppercase tracking-widest border shadow-sm ${
                    mill.status === 'SETTLED' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 
                ...
                <div className="grid grid-cols-2 gap-3 md:gap-4 mt-3 md:mt-4 pt-3 md:pt-4 border-t border-dashed border-slate-100 dark:border-white/5">
                  ...
                  <div className="text-right">
                    <p className="text-[7px] md:text-[8px] font-black text-orange-500 uppercase tracking-widest mb-1 flex items-center justify-end gap-1">
                      <Clock className="w-2 md:w-2.5 h-2 md:h-2.5" /> Pending
                    </p>
                    <p className="text-xs md:text-sm font-black text-slate-700 dark:text-slate-200 uppercase tracking-tight">{mill.pendingLots}</p>
                  </div>
                </div>
